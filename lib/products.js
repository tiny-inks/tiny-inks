...
/* Money in a SUMMARY column (cart totals, order summary) always carries two
   decimals, so subtotal / delivery / total line up digit-for-digit and match
   what checkout shows. Browsing prices keep formatPrice's whole numbers —
   "AED 65" reads better on a product card than "AED 65.00". */
export function formatMoney(amount, currency = 'AED', locale = 'en') {
  try {
    return new Intl.NumberFormat(locale === 'ar' ? 'ar-AE' : 'en-AE', {
      style: 'currency',
      currency,
