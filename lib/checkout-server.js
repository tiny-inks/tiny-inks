...
import { LOCAL_DIR, ADMIN_LIVE, ADMIN_DOMAIN, ADMIN_VERSION, addDemoOrder, adminFetch, adminRequest, redactSecrets } from '@/lib/print-server';
...
/* `quantityAvailable` needs the Storefront token to carry the
   `unauthenticated_read_product_inventory` scope. Without it Shopify rejects
   the WHOLE query, which used to take the entire checkout down with a 502
   ("Connection problem" in the UI) even though every other field was fine.
   So we ask for it, and fall back to the same query without it if the store
   won't give it to us — `availableForSale` alone still stops us selling
   anything that is out of stock, we just can't cap the quantity. */
const variantNodesQuery = (withInventory) => `
  query Variants($ids: [ID!]!) {
    nodes(ids: $ids) {
      ... on ProductVariant {
        ...
        title
        availableForSale
        ${withInventory ? 'quantityAvailable' : ''}
...
/* remembered per server instance so we only pay for the failed probe once */
let inventoryScope = true;

async function fetchVariantNodes(ids) {
  if (inventoryScope) {
    try {
      return await shopifyFetch(variantNodesQuery(true), { ids });
    } catch (e) {
      console.warn('quantityAvailable unavailable (storefront inventory scope missing?) — continuing without it:', e.message);
      inventoryScope = false;
    }
  }
  return shopifyFetch(variantNodesQuery(false), { ids });
}
...
async function resolveVariantsLive(items) {
  ...
  const out = [];
  if (realItems.length) {
    const data = await fetchVariantNodes(realItems.map((i) => i.variantId));
...
export async function createPaymentIntent({ amountFils, payload, locale }) {
  return stripeFetch('payment_intents', {
    ...
    /* Card only — on purpose. Apple Pay and Google Pay ARE card payments, so
       they still work (offered as branded buttons by the Express Checkout
       Element); this only drops Link, whose inline "save my info for faster
       checkout" signup turned the card form into a long, confusing block.
       Swap this line back to 'automatic_payment_methods[enabled]': 'true' to
       re-enable Link and anything else switched on in the Stripe dashboard. */
...
/* ------------------------------------------------------------ charged-price snapshot
   The Shopify order must record what Stripe ACTUALLY charged. Product prices,
   the delivery fee or the free-delivery threshold can change between creating
   the PaymentIntent and its webhook arriving (a redeploy, a price edit, a Stripe
   retry days later). So the intent route stores the exact breakdown it charged
   in the PaymentIntent metadata, and the webhook builds the order from that.
   All amounts in fils; short keys because metadata space is limited:
   m = method, u = unit price per quote line, q = qty per line,
   s = subtotal, d = delivery, t = total (= the PaymentIntent amount). */
export function pricingSnapshot(quote) {
  return {
    v: 1,
    m: quote.method,
    u: quote.lines.map((l) => l.unitFils),
    q: quote.lines.map((l) => l.qty),
    s: quote.subtotalFils,
    d: quote.deliveryFils,
    t: quote.totalFils,
  };
}

/* fresh quote (titles, flags) + snapshot (what was charged) → the quote the order
   is created from. Refuses (stage 'amount_mismatch') rather than inventing numbers. */
export function chargedQuote(quote, snapshot, chargedFils) {
  const refuse = (why) => Object.assign(new Error(`amount_mismatch: ${why}; Stripe charged ${chargedFils} fils`), { stage: 'amount_mismatch' });
  if (!snapshot) {
    /* PaymentIntent created before snapshots existed: today's quote is only
       usable if it still adds up to exactly what was charged */
    if (quote.totalFils !== chargedFils) throw refuse(`no price snapshot and today's quote is ${quote.totalFils} fils`);
    return quote;
  }
  const { m, u, q, s, d, t } = snapshot;
  const ints = (a) => Array.isArray(a) && a.every((n) => Number.isInteger(n) && n >= 0);
  if (!ints(u) || !ints(q) || u.length !== quote.lines.length || q.length !== quote.lines.length) throw refuse('price snapshot lines do not match the order lines');
  if (quote.lines.some((l, i) => l.qty !== q[i])) throw refuse('price snapshot quantities do not match the order lines');
  if (m !== quote.method) throw refuse('price snapshot delivery method differs');
  const lines = quote.lines.map((l, i) => ({ ...l, unitFils: u[i], totalFils: u[i] * l.qty }));
  const lineSum = lines.reduce((sum, l) => sum + l.totalFils, 0);
  if (lineSum !== s || !Number.isInteger(d) || d < 0 || s + d !== t || t !== chargedFils) {
    throw refuse(`price snapshot does not add up (lines ${lineSum}, subtotal ${s}, delivery ${d}, total ${t})`);
  }
  if (quote.totalFils !== t) console.warn(`checkout: prices changed since payment (today ${quote.totalFils} fils, charged ${t} fils) — order uses the charged amounts`);
  return { ...quote, lines, subtotalFils: s, deliveryFils: d, totalFils: t };
}
...
export async function findOrderForKey(key) {
  ...
  if (ADMIN_LIVE) {
    ...
    } catch (e) { console.error(`order lookup failed (stage=${e.stage || 'shopify_order_lookup'} status=${e.status || 'n/a'}):`, redactSecrets(e.message, 600)); }
  }
  return null;
}
...
export async function createOrderFromPayload(payload, quote, payment) {
  ...
  const line_items = quote.lines.map((l) => {
    ...
    return {
      ...
      quantity: l.qty,
      /* the unit price actually charged — without it Shopify prices the line at
         the variant's price at the moment the order is created */
      price: toAed(l.unitFils).toFixed(2),
  ...
  const order = {
    ...
    shipping_address: quote.method === 'delivery' ? {
      ...
      /* the customer's real city (captured by the map picker); omitted rather
         than guessed — we deliver UAE-wide, so defaulting to Abu Dhabi put a
         wrong city on every order outside the emirate */
  ...
  /* no PII here: variant ids, quantities, amounts and flags only */
  const safeRequest = {
    host: ADMIN_DOMAIN,
    apiVersion: ADMIN_VERSION,
    endpoint: 'POST /orders.json',
    lineItems: line_items.map((li) => ({ variant_id: li.variant_id, quantity: li.quantity, price: li.price, properties: li.properties.length })),
    method: quote.method,
    hasShippingAddress: Boolean(order.shipping_address),
    hasCity: Boolean(order.shipping_address?.city),
    financial_status: order.financial_status,
    transactionAmount: order.transactions?.[0]?.amount ?? null,
    shippingPrice: order.shipping_lines?.[0]?.price ?? null,
    currency: order.currency,
    tags: order.tags,
  };
  if (line_items.some((li) => !li.variant_id)) {
    console.error('SHOPIFY ORDER CREATE: line item without a numeric variant_id', JSON.stringify(safeRequest));
  }

  const r = await adminRequest('/orders.json', { method: 'POST', body: JSON.stringify({ order }) });
  const text = await r.text();
  let j = {};
  try { j = JSON.parse(text); } catch { /* HTML/empty body — reported raw below */ }
  if (!r.ok || !j.order) {
    const requestId = r.headers.get('x-request-id') || '';
    const shopifyBody = j && (j.errors || Object.keys(j).length) ? (j.errors || j) : (text.slice(0, 500) || '(empty body)');
    console.error('SHOPIFY ORDER CREATE REJECTED', JSON.stringify({
      status: r.status,
      requestId,
      contentType: r.headers.get('content-type') || '',
      body: redactSecrets(shopifyBody, 1500),
      request: safeRequest,
    }));
    const e = new Error(`Shopify order create failed (${r.status})${requestId ? ` request-id=${requestId}` : ''}: ${redactSecrets(shopifyBody, 500)}`);
    e.status = r.status;
    e.stage = 'shopify_order_create';
    throw e;
  }
  /* Shopify computes the order total itself. It must equal what was paid; if it
     ever does not (e.g. a line price was not honoured), say so loudly */
  const shopifyTotalFils = j.order.total_price != null ? toFils(j.order.total_price) : null;
  if (shopifyTotalFils != null && shopifyTotalFils !== quote.totalFils) {
    console.error('SHOPIFY ORDER TOTAL MISMATCH', JSON.stringify({ order: j.order.name, shopifyTotalFils, paidFils: quote.totalFils, key }));
    if (ALERT_URL) {
      fetch(ALERT_URL, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text: `⚠️ tinyinks.ae: Shopify order ${j.order.name} total ${toAed(shopifyTotalFils)} AED ≠ paid ${toAed(quote.totalFils)} AED (${key})` }),
      }).catch(() => {});
    }
  }
...
export function cleanPayload(body, locale) {
  ...
  return {
    ...
    address: { line: cleanStr(body.address?.line, 500), city: cleanStr(body.address?.city, 80), lat: cleanStr(body.address?.lat, 20), lon: cleanStr(body.address?.lon, 20) },
