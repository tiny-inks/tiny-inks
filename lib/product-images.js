...
/* Image for a CART LINE. A Shopify cart line only carries the variant's photo,
   which is null both when no per-variant image was set AND when the product has
   no photos at all — and a good number of the real catalogue's products have
   none. Without this the cart showed a blank placeholder for items that display
   a category photo everywhere else on the site, which just looked broken. */
export function cartLineImage(item) {
  if (!item) return null;
  if (item.image) return item.image;
  return fallbackImage({ handle: item.handle, title: item.title })?.url || null;
}
