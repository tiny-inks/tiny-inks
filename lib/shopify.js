...
export function normalizeCart(cart) {
  ...
  return {
    ...
    items: (cart.lines?.edges || []).map((e) => ({
      ...
      /* the variant's own image is null whenever the merchant (or an import)
         never set a per-variant photo — very common for single-variant
         products. Fall back to the product's own featured image so the cart
         doesn't show a blank placeholder for a product that clearly has a
         real photo everywhere else on the site. */
      image: e.node.merchandise.image?.url || e.node.merchandise.product.featuredImage?.url || null,
      qty: e.node.quantity,
