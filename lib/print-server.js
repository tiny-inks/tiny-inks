...
export async function getAdminToken(force = false) {
  ...
  const r = await fetch(`https://${ADMIN_DOMAIN}/admin/oauth/access_token`, {
    method: 'POST',
  ...
  if (!r.ok || !j.access_token) throw new Error(`Admin token grant failed (${r.status}): ${JSON.stringify(j).slice(0, 200)}`);
